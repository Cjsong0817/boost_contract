    ...
    <b-card title=" ">
      <b-form class="mx-5" v-on:submit.prevent="saveConfig">
        ...
        >
          ...
          <div class="col-sm-6" v-else-if="item.global_key == 'SUPPORT_QR_CN'">
            <router-link
              :to="'/upload' + dataset[item.global_key]+'.jpg'"
              target="_blank"
              v-show="!displayCN"
            >
              <img
                :src="
                  origin +
                  '/upload' +
                  dataset[item.global_key]+'.jpg'
                "
                @error="imageLoadErrorCN"
                height="50"
              />
            </router-link>
            <b-form-file
          ...
          <div class="col-sm-6" v-else-if="item.global_key == 'SUPPORT_QR_EN'"><router-link
              :to="'/upload' + dataset[item.global_key]+'.jpg'"
              target="_blank"
              v-show="!displayEN"
            >
...
export default {
  ...
  methods: {
    ...
    setImageEN(e) {
      ...
      console.log(file);
      this.dataset["SUPPORT_QR_EN"] = file;
      console.log(this.dataset["SUPPORT_QR_EN"]);
    ...
    setImageCN(e) {
      const file = e.target.files[0];
      this.dataset["SUPPORT_QR_CN"] = file;
      if (!file.type.includes("image/")) {
        alert("Please select an image file");
        return;
      }
