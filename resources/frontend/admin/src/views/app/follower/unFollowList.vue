<template>
  <div class="main-content">
    <breadcumb :page="$t('unFollowList')" :folder="$t('insuranceManage')" />
  ...
  selectionInfoClass: 'table-alert__box',
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <span v-else-if="props.column.field == 'action'">
            <button :disabled="isLoading" v-if="props.row.status == 0" type="submit"
              class="btn btn-default btn-edit py-3" @click="showModalApprove(props.row)">
              <span>{{ $t("approve") }}</span>
            </button>

            <button :disabled="isLoading" v-if="props.row.status == 0" type="submit"
              class="btn btn-default btn-delete py-3 ml-1" @click="showModalReject(props.row)">
              <span>{{ $t("reject") }}</span>
            </button>
          </span>
        </template>
...
export default {
  ...
  methods: {
    ...
    approved(row) {
      ...
      var result = followControl(formData);
      var self = this;
      result
        .then(function (value) {
