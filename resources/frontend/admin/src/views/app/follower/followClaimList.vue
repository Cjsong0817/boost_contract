<template>
  <div class="main-content">
    <breadcumb :page="$t('followClaimList')" :folder="$t('insuranceManage')" />
    ...
    <b-tabs pills align="end" v-model="tabIndex">
      <b-tab :title="$t('claim_list')" active @click="clearData">
        <b-card :title="$t('claim_list')">
          <div class=" text-right d-block mb-4">
            <button :disabled="isLoading" type="button" class="btn btn-default btn-success" @click="claimAll()">
  ...
  selectionInfoClass: 'table-alert__box',
            ...
            <template slot="table-row" slot-scope="props">
              ...
              <span v-else-if="props.column.field == 'action'">
                <!-- <button :disabled="isLoading" type="submit" class="btn btn-default btn-edit py-3"
                  @click="showModalApprove(props.row)">
                  <span>{{ $t("approve") }}</span>
                </button>

                <button :disabled="isLoading" type="submit" class="btn btn-default btn-delete py-3 ml-1"
                  @click="showRejectModal(props.row)">
                  <span>{{ $t("reject") }}</span>
                </button> -->
                <button :disabled="isLoading" type="submit" class="btn btn-default btn-edit py-3 ml-2"
      ...
      <b-tab :title="$t('insurance') + '(' + searchUsernameWallet + ')'" :title-item-class="{ hiddenClass: !canShow }">
          ...
          <div class="align-items-center mobile-adjust">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-end">
              <b-pagination class="pagi-margin pt-3" v-model="walletPageNumber" :total-rows="totalWalletRecords"
                ...
                :last-text="$t('last')" @input="_getBetRecordList">
              </b-pagination>

              <b-input-group class="ml-3" style="width: 160px">
                <b-form-input v-model="walletPageNumber" :placeholder="$t('PageNo')"></b-form-input>
                <b-input-group-append>
                  <b-button variant="primary" @keypress="_getBetRecordList" @click="_getBetRecordList">{{
                    $t("go")
                  }}</b-button>
                </b-input-group-append>
              </b-input-group>
...
export default {
  ...
  methods: {
    ...
    claimAll() {
      this.$swal({
        title: this.$t("are_you_sure_to_claim_all"),
        ...
        confirmButtonText: this.$t("claimAll")
      }).then(result => {
        if (result.value) {
          console.log(result.value);
          var _result = followerClaim();
          var self = this;
          _result
            .then(function (value) {
