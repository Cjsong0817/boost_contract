<template>
  <div class="main-content">
    <breadcumb :page="$t('level_bonus')" :folder="$t('bonusRecord')" />
    <b-row>
      <b-col md="12">
        <b-card class="mb-30">
          <b-row align-v="center">
            ...
            <b-col md="1" class="mt-3 mt-md-0" v-if="totalRecords>0">
              <router-link :to="'/admin-api/export/exportBonus?token='+adminToken+'&username='+searchUsername+'&bonus=level_bonus&from='+from+'&to='+to+''" target="_blank">
                  {{$t("export")}}
              </router-link>
...
if(dHour.toString().length == 1){
    ...
    loadItems() {
      ...
      var result = getBonusRecord("level_bonus", this.searchUsername, this.from, this.to, this.pageNumber);
      var self = this;
      this.isLoading = true;
      result
        .then(function (value) {
