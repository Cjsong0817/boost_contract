<template>
  <div class="main-content">
    <breadcumb :page="$t('revert_claim')" :folder="$t('insuranceManage')" />
...
export default {
  ...
  methods: {
    ...
    showModalApprove(row) {
      this.$swal({
        title: this.$t("are_you_sure_to_revert"),
        ...
        confirmButtonText: this.$t("revert"),
      }).then((result) => {
        if (result.value) {
    ...
    approved(row) {
      ...
      var result = revertControl(formData);
      var self = this;
      result
        .then(function () {
