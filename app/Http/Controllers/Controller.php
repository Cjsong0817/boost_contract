...
class Controller extends BaseController
{
    ...
    public function search_record($db, $request)
    {
        ...
        if ($request->get('status')) {
            if ($request->get('status') != null || $request->get('status') != 'null') {
                $db->where('status', $request->get('status'));
            }
        }
        return $db;
    }
    public function isBEP20Address($address)
    {
        // BEP-20 地址长度为 42 个字符（不包括 0x 前缀）
    ...
    public function create_transaction($user_id, $act, $wallet, $found_type, $wallet_type, $amount, $detail, $detailen, $detailvn = '', $detailin = '', $detailth = '', $remark = '')
    {
    ...
    public function wallet_type()
    {
        return array
            (
            1 => array('id' => "point1", 'found_type' => "1", 'lan_key' => "POINT1", 'comments_cn' => "USDT", 'comments_en' => "USDT"),
            2 => array('id' => "point2", 'found_type' => "2", 'lan_key' => "POINT2", 'comments_cn' => "基金余额", 'comments_en' => "Earning balance"),
            3 => array('id' => "point3", 'found_type' => "3", 'lan_key' => "POINT3", 'comments_cn' => "团队奖金", 'comments_en' => "Active Earning"),
            4 => array('id' => "point4", 'found_type' => "4", 'lan_key' => "POINT4", 'comments_cn' => "会员奖金", 'comments_en' => "MGM Reward"),
            //5 => array('id' => "point5", 'found_type' => "5", 'lan_key' => "POINT5", 'comments_cn' => "Smartled Credit", 'comments_en' => "Smartled Credit"),
            //3 => array('id' => "point3", 'found_type' => "3", 'lan_key' => "POINT3", 'comments_cn' => "ETNEX", 'comments_en' => "ETNEX"),
    ...
    public function fount_type()
    {
        return array
            (
            0 => array('found_type' => "1001", 'action' => "bonus", 'comments_cn' => "直推收益", 'comments_en' => "Sponsor Bonus"),
            1 => array('found_type' => "1002", 'action' => "bonus", 'comments_cn' => "质押收益", 'comments_en' => "Roi Bonus"),
            ...
            10 => array('found_type' => "902", 'action' => "tradeprofit", 'comments_cn' => "跟单交易盈利", 'comments_en' => "Follow Trade Profit"),
    ...
    public function bonus_type()
    {
        return array
            ...
            //  0 => array('key' => "sponsor_bonus", 'bonus_cn' => "直推收益", 'bonus_en' => "Sponsor Bonus"),
            0 => array('key' => "static_bonus", 'bonus_cn' => "基金收益", 'bonus_en' => "Fund Reward"),
            1 => array('key' => "dynamic_bonus", 'bonus_cn' => "会员分享奖金", 'bonus_en' => "MGM Reward"),
            2 => array('key' => "sponsor_bonus", 'bonus_cn' => "介绍人奖金", 'bonus_en' => "Dealer introducer Bonus"),
            3 => array('key' => "level_bonus", 'bonus_cn' => "团队奖金", 'bonus_en' => "Dealer Group Bonus"),
            //  3 => array('key' => "level_bonus", 'bonus_cn' => "同级奖励", 'bonus_en' => "Same Rank Bonus"),

        );
    }
    public function containsChinese($variable)
    {
        // 使用正则表达式匹配中文字符
        if (preg_match('/[\x{4e00}-\x{9fa5}]/u', $variable)) {
            // 如果匹配到了中文字符，返回 true
            return true;
        } else {
            // 如果没有匹配到中文字符，返回 false
            return false;
        }
    }
...
<table border='0' cellpadding='0' cellspacing='0' width='100%'>
    <tr>
        <td align='center' valign='top' width='100%' background='bg.jpg' bgcolor='#f6f6f6' style='background: #f6f6f6 url(bg.jpg); background-size: cover; padding: 35px 15px 0 15px;' class='mobile-padding'>
            ...
            <![endif]-->
            <img  src='" . url('') . "/images/system_logo.png'  width='25%' border='0' style='display: block;height:210px;width:280px'>
            <br><br>
        </td>
    </tr>
    <tr>
                                        ...
                                        <tr>
                                            <td align='left' style='font-family: Open Sans, Helvetica, Arial, sans-serif;'>
                                                ...
                                                <p style='margin-top:0in;text-align: left;margin-right:0in;margin-bottom:8.0pt;margin-left:0in;line-height:107%;font-size:15px;'>
                                                    <span style='font-size:19px;line-height:107%;font-family:'Times New Roman','serif';'> Your Verification code for Winato is $content</span>
                                                </p>
                                            </td>
                                        </tr>
                                    </table>
...
"; //邮件内容
        ...
        $param = array(
            ...
            'apiKey' => $API_KEY,
            'from' => 'support@winato.com', # 发信人，用正确邮件地址替代
            'fromName' => 'support@winato.com',
